...
def main(args):
    ...
    # Load SSL model
    if args.feat_cond:
        # ssl_model = get_model(args.type_model, args.use_head, reconstruct=True).to(args.gpu).eval()
        ssl_model = get_model(args.type_model, args.use_head, reconstruct=True).to(args.gpu).eval()
...
def load_ssl_data(args, ssl_model=None):
    ...
    print(f"args.image_size: {args.image_size}")
    for batch_small, batch_big, model_kwargs, masks_enc, masks_pred, masks_C, masks_eC in data:
        # We add the conditioning in conditional mode
        if ssl_model is not None:
            print(f"batch_small shape: {batch_small.shape}")
            print(f"batch_big shape: {batch_big.shape}")
            with th.no_grad():
                with autocast(args.use_fp16):
                    # we always use an image of size 224x224 for conditioning
                    batch_big = batch_big.to(args.gpu)
                    masks_enc = [u.to(args.gpu) for u in masks_enc]
                    masks_pred = [u.to(args.gpu) for u in masks_pred]
                    start_time = time.time()
                    model_kwargs["feat"] = ssl_model(
                                                batch_big, 
                                                masks_enc,
                                                masks_pred,
                                            ).detach()
                    print("---SSL compute time %s seconds ---" % (time.time() - start_time))
                    # print(model_kwargs["feat"])
            yield batch_small, model_kwargs
        else:
            yield batch_small, model_kwargs
