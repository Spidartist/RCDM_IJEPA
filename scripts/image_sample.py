...
def main(args):
    ...
    ssl_model = get_model(args.type_model, args.use_head).to(device, non_blocking=True).eval()
    for p in ssl_model.parameters():
        ssl_model.requires_grad = False
    ssl_dim = ssl_model(th.zeros(1,3,224,224).to(device, non_blocking=True)).size(1)
