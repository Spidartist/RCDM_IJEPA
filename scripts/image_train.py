...
def load_ssl_data(args, ssl_model=None):
    ...
    # for batch, batch_big, model_kwargs, masks_enc, masks_pred, masks_C, masks_eC in data:
        # We add the conditioning in conditional mode
        if ssl_model is not None:
            print(f"batch_small type: {batch.type()}")
            print(f"batch_big type: {batch_big.type()}")
            with th.no_grad():
