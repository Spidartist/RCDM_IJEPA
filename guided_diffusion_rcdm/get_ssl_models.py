...
class Wrapper(nn.Module):
    ...
    """
    def __init__(self, model, head, use_head=False, predictor=None, reconstruct=False):
        super().__init__()
        self.head = head
        self.model = model
    ...
    def forward(self, x, masks_enc=None, masks_pred=None):
        print(f"x.shape: {x.shape}")
        if not self.reconstruct:
            x = self.model(x)
            print(f"Output shape: {x.shape}")
        elif self.reconstruct and masks_enc is not None and masks_pred is not None:
            x = self.model(x, masks_enc[0])
            x = self.predictor(x, masks_enc[0], masks_pred[0])
            print(f"Output shape RC: {x.shape}")
...
def get_model(model="dino", use_head=False, reconstruct=False):
    '''
    ...
    if model == "supervised":
        ...
        if not reconstruct:
            ...
            print(msg)
            print(msg2)
            return Wrapper(embedding_model, None, use_head=False, predictor=predictor, reconstruct=reconstruct)
    else:
